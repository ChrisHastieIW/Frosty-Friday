...
def upload_files(
  ...
  uploaded_files = st.file_uploader(label = f"Select file to ingest into {schema}.{table}", type = ["csv"], accept_multiple_files = True)

  ### Write waiting message until files are uploaded
  if uploaded_files is None or len(uploaded_files) == 0 :
    st.write("Awaiting file(s) to upload...")
  else :
    ...
    for uploaded_file in uploaded_files:
      try :
        ...
        uploaded_df = pd.read_csv(bytes_data, sep = ',', header = 0, quotechar = '"')
        ...
        st.write(f'Your upload of file "{uploaded_file.name}" was a success. You uploaded {len(uploaded_df)} rows.')

      except Exception as e:
        st.write(f'Error during upload of file "{uploaded_file.name}"\n\n{e}')
...
def streamlit_app() :
  ...
  st.write('# Manual CSV to Snowflake Table Uploader')

  ### Build schema toggle
  schema_toggle = st.radio(label = 'Select schema:', options = schemas_df)
  ...
  ### Build table toggle
  table_toggle = st.radio(label = 'Select table to upload to:', options = filtered_tables_df)
